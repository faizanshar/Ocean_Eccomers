...
import {styles} from './Stylescreenstore';
import AsyncStorage from '@react-native-community/async-storage';
import LinearGradient from 'react-native-linear-gradient';

// import {TextInput} from 'react-native-gesture-handler';

export default class Screenstore extends Component {
  constructor(props) {
    super(props);
    this.state = {
      dataSource: [],
  ...
  getUser() {
    fetch('https://api-oceanstore.herokuapp.com/api/auth', {
      ...
      .then((responseJson) => {
        console.log('ini response json user ', responseJson);
        const {status} = responseJson;
        if (status) {
          alert(status);
        } else {
          this.setState({dataSource2: responseJson.user, refresh: false});
  ...
  getProduct() {
    fetch('https://api-oceanstore.herokuapp.com/api/seller', {
      ...
      .then((response) => response.json())
      .then((responseJson) => {
        console.log('ini response json ', responseJson);
        const {status} = responseJson;
        if (status) {
          alert(status);
        } else {
  ...
  deleteProduct(index) {
    fetch(
      `https://api-oceanstore.herokuapp.com/api/seller/${this.state.dataSource[index].id}`,
      {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
    ...
    return (
      <View style={styles.container}>
        <View style={styles.containerheader}>
          ...
          <Text style={styles.txtmystore}>MyStore</Text>
        ...
        <ScrollView
          ...
          <View style={styles.containerprofile}>
            <View style={styles.viewimgname}>
              <TouchableOpacity>
                <Image
                  source={
                    this.state.dataSource2.avatar_url
                      ? {
                          uri: this.state.dataSource2.avatar_url,
                        }
                      : {
                          uri:
              ...
              <View>
                <Text style={styles.txtname}>
                  {this.state.dataSource2.store}
                </Text>
                <View style={styles.containerprofile2}>
                  <Image
                    style={styles.imgmap}
                    source={require('../assets/map.png')}
                  />
                  <Text style={styles.txtmap}>
                    {this.state.dataSource2.store_address}
                  </Text>
                </View>
              </View>
            </View>
          ...
          {this.state.dataSource.length === 0 ? (
            <View style={styles.containerproduct}>
              <Image
                style={styles.imgnothing}
                source={require('../assets/nothing.jpg')}
              />
              <Text style={styles.txtnothing}>No item to sell</Text>
            </View>
          ) : (
            <View style={styles.containerproduct}>
        ...
        <TouchableOpacity
          ...
          <Image
            style={styles.img}
            source={require('../assets/blackplus.png')}
          />
        </TouchableOpacity>
      </View>
    );
